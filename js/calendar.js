        ...
        for (const c of this.cells()) {
            ...
            grid.append(el('div', {
                ...
                el('div', { class: 'cal-head-row' }, [
                    ...
                    // 會比那個月真正有事的那幾天還顯眼。
                    //
                    // **排班模式時整顆不畫。** 排班是連續點格子（一天一天點過去），
                    // 這顆按鈕平常靠 opacity:0 藏起來，但還是吃得到點擊——
                    // 手指點快了難免歪到那個角落，點到「加行程」而不是排上班。
                    // 那幾秒鐘裡它的位置只會添亂，不畫最乾脆。
                    this.shiftMode ? null : el('button', {
                ...
                /* 課不寫字，只留日期旁邊那幾個點。
                 *
                 * 她的原話：「太密密麻麻了，刪掉一些字　不然很亂」。
                 *
                 * **「2 堂課」跟上面那排點是同一個資訊講兩次**——點本來就
                 * 一堂一個。而那三個字在手機上會折成兩三行，把每一格撐高，
                 * 二十幾格全部長一樣，整張月曆變成一面字牆。
                 * 月曆要回答的是「這個月哪幾天不一樣」，重複二十次的東西
                 * 正好是最不需要被看見的那個。
                 *
                 * **篩了某一個分類還是整堂寫出來。** 她的原話是
                 * 「比如我按學校 就可以完全顯示課表」——篩選之後格子裡
                 * 只剩那一類，量少了就有空間，而且那時候她要看的
                 * 就是「這個月的學校課長什麼樣」。
                 *
                 * 上什麼課滑過去看得到（點上有 title），點下去底下那塊有完整的。
                 */
                ...(Agenda.filter
